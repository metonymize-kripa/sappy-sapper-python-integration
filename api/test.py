...
def make_range_response(symbol, resp_dict):
    ...
    if resp.ok: #Good response from FastAPI
        ...
        if 'error' in input_dict:
            ...
            return resp_dict
        resp_dict['symbol'] = symbol
        resp_dict['main_point'] = '$%s - $%s<a class="card-button text-white bg-primary bd-dark" on:click={getAPIData("call",%s)} href="">sell call</a>'%(round(input_dict["low_range"]),round(input_dict["high_range"]),symbol)
