...
<script>
	...
	const demo_tickers = ["SPY DIV", "T CALL", "TSLA DOOM", "GME WSB", "PLTR WISE", "GME VOLUME", "PLTR KELLY", "ARKK"];
	...
	let cached_tickers = demo_tickers.slice(0,4);
	let batch_commands = ["call", "wise", "div"];
	let selected_ticker = "";
	let wysiwyg_ticker = "";
	...
	function skillType(query, batch_commands) {
		...
		if ( query.toLowerCase().includes("call") ||  query.toLowerCase().includes("wise") ||  query.toLowerCase().includes("div")) {
			return "Slow skill: "
		}
		else {
			return "Quick skill: "
